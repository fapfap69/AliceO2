...
class Clusterer2
{
 ...
 public:
  ...
  Clusterer2(const Clusterer2&) = delete;
  Clusterer2& operator=(const Clusterer2&) = delete;

  static void Dig2Clu(gsl::span<const o2::hmpid::Digit> digs, std::vector<o2::hmpid::Cluster>& clus, float* pUserCut, bool isUnfold = kTRUE); // digits->clusters                                                                       // check for sigma cut
  static void regionGrowing(const mapCharge_t &pQMap, mapIndex_t &pIdxMap, DigCoord seed, std::vector<size_t> &theCluster, DigCoord &localMax);
